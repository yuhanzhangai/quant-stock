...
def test_kill_switch_skips_in_window_but_not_pending():
    df = cands({"signal_id": "in_win"}, {"signal_id": "pend", "call_ts": datetime(2026, 6, 9, 13, 0, tzinfo=UTC)})
...
def test_dedup_same_handle_issuer_keeps_latest():
    ...
    df = cands(
        {"signal_id": "old", "ticker": "GOOG", "call_ts": MON},
        {"signal_id": "new", "ticker": "GOOGL", "call_ts": datetime(2026, 6, 8, 15, 0, tzinfo=UTC)},
    )
    got = run(df)
...
def test_tradability_gates():
    df = cands(
        {"signal_id": "no_price", "ticker": "ZZZZ"},
...
def test_merge_same_issuer_higher_wilson_wins():
    df = cands(
        {"signal_id": "lo", "handle": "alice", "ticker": "GOOG"},
...
def test_settled_cash_sequential_depletion():
    ...
    df = cands(
        {"signal_id": "first", "handle": "bob", "ticker": "TSLA"},
...
def test_priority_conviction_tiebreak_on_equal_wilson():
    df = cands(
        {"signal_id": "hi_conv", "handle": "alice", "ticker": "GOOG", "conviction": "high"},
...
def test_rule_version_stamped_on_every_row():
    ...
    out = decide(df, decision_ts=DECISION_TS, pdt=PdtState(0, 100_000.0), prices={"AAPL": 50.0}, params=RuleParams())
