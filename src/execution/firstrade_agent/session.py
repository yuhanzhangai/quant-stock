...
class FirstradeSession:
    ...
    @staticmethod
    def _profile_lock_holder(profile_dir: Path) -> int | None:
        """专用 profile 是否正被一个活着的 Chrome 进程占用?返回持锁 pid,空闲则 None。

        Chrome 在 user-data-dir 根放 `SingletonLock` 符号链接(target=`主机名-pid`)。
        若链接指向的进程还活着 = 被占用(Playwright 此时接管会失败/抢登录态)。
        指向死进程 = 崩溃残留的陈旧锁,放行(Chrome 自己也会清)。"""
        lock = profile_dir / "SingletonLock"
        try:
            target = os.readlink(lock)  # 不存在 → OSError,即未占用
        except OSError:
            return None
    ...
    def launch(self) -> None:
        ...
        persistent = s.chrome_profile_dir is not None
        # 占用检测必须在起 playwright 之前:profile 被活着的 Chrome 持锁时,
        # launch_persistent_context 会抢登录态/失败 —— 先明确报错让 operator ⌘Q 退出。
        if persistent and s.chrome_profile_dir.exists():
            holder = self._profile_lock_holder(s.chrome_profile_dir)
            if holder is not None:
                raise ExecutionHalted(
                    f"专用 Chrome profile 正被进程 {holder} 占用: {s.chrome_profile_dir}。"
                    "请先 ⌘Q 完全退出用该 profile 打开的 Chrome,再重试(不要只关窗口)。"
                )
        ...
        try:
            ...
            if persistent:
                ...
                s.chrome_profile_dir.mkdir(parents=True, exist_ok=True)
                context = pw.chromium.launch_persistent_context(
                    str(s.chrome_profile_dir),
                    headless=s.headless,
                ...
                auth_reused = any(s.chrome_profile_dir.iterdir())
            else:
