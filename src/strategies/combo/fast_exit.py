...
class FastExitStrategy(StrategyBase):
    ...
    def generate_signals(self, price, fast_ma=90, profit_thr=0.3,
                         trend_ma=180, sl=2.0, tp=8.0, gap=144, **kw):
        strat = MinuteSwingStrategy()
        entries, _ = strat.generate_signals(price, trend_ma=trend_ma,
            stop_pct=sl, take_profit_pct=tp, min_gap=gap)
        ...
        for i in range(len(price)):
            if entries.iloc[i]:
                ep = price.iloc[i]; in_t = True
            elif in_t and ep > 0:
                pnl = (price.iloc[i] - ep) / ep * 100
                if pnl < -sl or pnl > tp or price.iloc[i] < ma_slow.iloc[i]:
                    exits.iloc[i] = True; in_t = False
                elif death.iloc[i] and pnl > profit_thr:
                    exits.iloc[i] = True; in_t = False
