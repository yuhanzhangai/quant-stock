...
def check_a1_orphan_fills(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    return [
        Finding(
            "A1",
            "halt",
            f"孤儿成交 {fill_id}:order_id 不存在于 orders",
            order_id=order_id,
            expected="orders 中存在",
            actual="缺失",
        )
...
def check_a2_overfill(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    rows = con.execute(
        ...
        "JOIN v_order_filled vf USING (order_id) WHERE vf.filled_qty > o.qty"
    ).fetchall()
    return [
        Finding("A2", "halt", "超额成交", ticker=t, order_id=oid, expected=f"filled ≤ {q}", actual=str(fq))
...
def check_a3_status_fill_consistency(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    rows = con.execute(
        ...
        "FROM v_orders_current o LEFT JOIN v_order_filled vf USING (order_id)"
    ).fetchall()
    with_partial_hist = {
        r[0] for r in con.execute("SELECT DISTINCT order_id FROM orders WHERE status = 'partial'").fetchall()
    }
...
def check_a4_stale_submitted(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    rows = con.execute(
        ...
        "JOIN v_order_filled vf USING (order_id) WHERE o.status = 'submitted'"
    ).fetchall()
    findings: list[Finding] = []
    for oid, ticker, first_fill_ts in rows:
        ...
        findings.append(
            Finding(
                "A4",
                sev,
                f"有成交但状态停在 submitted 已 {days} 个交易日",
                ticker=ticker,
                order_id=oid,
                actual=f"first_fill_ts={first_fill_ts}",
            )
        )
    return findings
    ...
    for oid, seqs, statuses, corrects in rows:
        ...
        for seq, status, corr in zip(seqs, statuses, corrects, strict=True):
            ...
            if terminal_at is not None:
                findings.append(
                    Finding("A5", "halt", f"终态(seq={terminal_at})后出现非更正行 seq={seq}({status})", order_id=oid)
                )
                break
...
def check_a6_relation_chain(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    findings = [
        Finding("A6", "halt", "订单引用不存在的 signal_id", order_id=oid, actual=sid)
        for oid, sid in con.execute(
            "SELECT DISTINCT o.order_id, o.signal_id FROM orders o "
            "WHERE NOT EXISTS (SELECT 1 FROM signals s WHERE s.signal_id = o.signal_id)"
        ).fetchall()
    ]
    findings += [
        Finding("A6", "halt", "开仓单的信号 decision 非 followed", ticker=t, order_id=oid, actual=dec)
        for oid, t, dec in con.execute(
            "SELECT o.order_id, o.ticker, s.decision FROM v_orders_current o "
            "JOIN signals s USING (signal_id) WHERE o.side = 'buy' AND s.decision <> 'followed'"
        ).fetchall()
    ]
    findings += [
        Finding("A6", "halt", "平仓单 exit_reason 为空", ticker=t, order_id=oid)
        for oid, t in con.execute(
            "SELECT order_id, ticker FROM v_orders_current WHERE side = 'sell' AND exit_reason IS NULL"
        ).fetchall()
    ]
    findings += [
        Finding("A6", "halt", "direction_flip 平仓缺 exit_trigger_signal_id", ticker=t, order_id=oid)
        for oid, t in con.execute(
            "SELECT order_id, ticker FROM v_orders_current "
            "WHERE exit_reason = 'direction_flip' AND exit_trigger_signal_id IS NULL"
        ).fetchall()
    ]
    return findings
...
def check_a7_timeline(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    findings = [
        Finding("A7", "warn", "信号 ingested_ts 早于 call_ts", actual=f"signal_id={sid}")
        for (sid,) in con.execute("SELECT signal_id FROM signals WHERE ingested_ts < call_ts").fetchall()
    ]
    findings += [
        Finding("A7", "warn", "submitted_ts 早于信号 ingested_ts", order_id=oid)
        for (oid,) in con.execute(
            "SELECT DISTINCT o.order_id FROM orders o JOIN signals s USING (signal_id) "
            "WHERE o.submitted_ts < s.ingested_ts"
        ).fetchall()
    ]
    findings += [
        Finding(
            "A7",
            "warn",
            "fill_ts 早于 submitted_ts 超容差",
            order_id=oid,
            actual=f"fill_id={fid}",
            expected=f"≥ submitted_ts − {cfg.a7_tolerance_min}min",
        )
        for fid, oid in con.execute(
            "SELECT f.fill_id, f.order_id FROM v_fills_effective f "
            "JOIN v_orders_current o USING (order_id) "
            f"WHERE f.fill_ts < o.submitted_ts - INTERVAL {int(cfg.a7_tolerance_min)} MINUTE"
        ).fetchall()
    ]
    return findings
...
def check_a8_sell_within_holdings(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    return [
        Finding("A8", "halt", "重放出现负持仓(卖超持有)", ticker=t, actual=f"fill {fid} 后净持仓 {run}")
        for t, fid, run in rows
    ]
...
def check_a9_watermark_freshness(con: duckdb.DuckDBPyConnection, cfg: ReconConfig, now: datetime) -> list[Finding]:
    ...
    if age_h > cfg.a9_max_age_hours:
        return [Finding("A9", "warn", f"水位陈旧 {age_h:.1f}h(阈值 {cfg.a9_max_age_hours}h)", actual=str(latest))]
    return []


CHECKS = (
    check_a1_orphan_fills,
    check_a2_overfill,
    check_a3_status_fill_consistency,
    check_a4_stale_submitted,
    check_a5_event_stream,
    check_a6_relation_chain,
    check_a7_timeline,
    check_a8_sell_within_holdings,
    check_a9_watermark_freshness,
)


def run_checks(
    con: duckdb.DuckDBPyConnection, cfg: ReconConfig | None = None, now: datetime | None = None
) -> ReconResult:
...
def run_recon(ledger_path: Path, cfg: ReconConfig | None = None, out_root: Path = REPORTS_ROOT) -> ReconResult:
    ...
    meta = {
        ...
        "scope": "ledger_only",  # A 组;B 组待页面能力(设计 §1.2)
    ...
    lines = [
        ...
        "",
        f"**裁决:{result.result.upper()}**(9 项检查,{result.checks_failed} 项有发现,{len(result.findings)} 条 finding)",
        "",
        "| check | severity | message | ticker | order_id |",
        "|---|---|---|---|---|",
    ]
    lines += [
        f"| {f.check_id} | {f.severity} | {f.message} | {f.ticker or '—'} | {f.order_id or '—'} |"
