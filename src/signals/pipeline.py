...
_OUT_SCHEMA = pl.Schema({
    "signal_id": pl.String, "tweet_id": pl.String, "handle": pl.String, "author_id": pl.String,
    "tier": pl.String, "tier_csv_date": pl.Date, "ticker": pl.String, "direction": pl.String,
    "call_ts": pl.Datetime("us", "UTC"), "ingested_ts": pl.Datetime("us", "UTC"),
    "tweet_text": pl.String, "tweet_url": pl.String, "tweet_created_at": pl.Datetime("us", "UTC"),
    "tweet_blocked": pl.Boolean, "conviction": pl.String, "confidence": pl.Float64,
})
...
def _read_snapshots(tweet_ids: list[str], db_path: Path) -> pl.DataFrame:
    ...
    try:
        ...
        con.close()
    return pl.DataFrame(rows, schema={"tweet_id": pl.String, "tweet_text": pl.String, "tweet_url": pl.String,
                                      "created_at_raw": pl.String, "blocked_raw": pl.Int64}, orient="row")
...
def build_candidates(
    ...
    *,
    ...
    flips = (calls.filter(pl.col("direction") != "bullish")
             .group_by("handle", "ticker").agg(pl.col("call_ts").max().alias("last_flip_ts")))
    ...
    if conflicted.height:
        logger.info("冲突过滤剔除 {} 条(批内同 handle×ticker 已翻向): {}",
                    conflicted.height, conflicted.select("handle", "ticker", "tweet_id").rows())
    cand = (bullish.filter(pl.col("last_flip_ts").is_null() | (pl.col("last_flip_ts") < pl.col("call_ts")))
            .drop("last_flip_ts"))
    if cand.is_empty():
        return pl.DataFrame(schema=_OUT_SCHEMA)
    ...
    ).select(_OUT_COLS).cast(_OUT_SCHEMA))
...
def persist_candidates(candidates: pl.DataFrame, db_path: Path = DEFAULT_SNAPSHOT_DB) -> int:
    ...
    try:
        ...
        con.execute(f"INSERT INTO signal_candidates ({cols}) SELECT {cols} FROM cand_batch "
        ...
        logger.info("signal_candidates 落库: 批内 {} / 新增 {} / 已存在 {}",
...
def run_pipeline(
    *,
    ...
    cands = build_candidates(calls, horizon=horizon, leaderboard_path=leaderboard_path,
