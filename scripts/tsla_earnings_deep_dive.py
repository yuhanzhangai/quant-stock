...
def analyze_earnings_window(
    ...
    if len(pre_event) > 0 and len(post_event) > 0:
        ...
        print(f"  财报后最高: ${post_event['high'].max():.2f} ({(post_event['high'].max() / pre_close - 1) * 100:+.2f}%)")
        print(f"  财报后最低: ${post_event['low'].min():.2f} ({(post_event['low'].min() / pre_close - 1) * 100:+.2f}%)")
        print(f"  最终收盘: ${post_event['close'].iloc[-1]:.2f} ({(post_event['close'].iloc[-1] / pre_close - 1) * 100:+.2f}%)")
    ...
    for label, ts in time_points:
        ...
        if event_price is not None and label != "事件时刻":
            ...
            print(f"  {label:>12}: ${price:.2f}  (vs窗口起: {ret_from_base:+.2f}%  vs事件: {ret_from_event:+.2f}%)  vol: {vol:.0f}")
        else:
    ...
    if len(post_event) > 1:
        ...
        event_price_val = post_prices.iloc[0]
    ...
    factors = detect_sentiment(
        window["close"], window["volume"], event_ts,
