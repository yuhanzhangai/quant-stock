...
for coin in COINS:
    ...
    for seg_i, (seg, label) in enumerate(zip(segs, seg_labels)):
        ...
        for typ, params, m in [('OLD', OLD_PARAMS[coin], m_old), ('NEW', NEW_PARAMS[coin], m_new)]:
            results.append({
                'coin': coin, 'segment': f"S{seg_i+1}({label})", 'type': typ,
                'tp': params['take_profit_pct'], 'gap': params['min_gap'],
                'trades': m['total_trades'], 'win_rate': m['win_rate_pct'],
                'return_pct': m['total_return_pct'], 'sharpe': m['sharpe_ratio'],
                'max_dd': m['max_drawdown_pct'], 'final_val': m['final_value'],
            })
...
for coin in COINS:
    ...
    print(f"  NEW: trend_ma={np_['trend_ma']}, stop={np_['stop_pct']}%, tp={np_['take_profit_pct']}%, gap={np_['min_gap']}")
    print(f"{'─' * W}")
    ...
    for seg_name in coin_df['segment'].unique():
        ...
        for _, row in seg_df.iterrows():
            ...
            print(f" {star} {row['segment']:^24} {row['type']:^5} {row['trades']:>4} "
...
print(f"{'=' * W}")
print(f"  {'Coin':^8} {'Type':^5} {'Trades':>6} {'AvgWR%':>7} {'TotRet%':>9} {'AvgSharpe':>9} {'5xTotRet%':>10} {'5xTotP&L$':>10} {'Result':>8}")
...
for coin in COINS:
    ...
    for typ in ['OLD', 'NEW']:
        ...
        print(f"  {coin:^8} {typ:^5} {tot_tr:>6} {avg_wr:>7.1f} {tot_ret:>+9.2f} "
