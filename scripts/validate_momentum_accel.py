...
PARAMS = {
    'ETH':  dict(velocity_window=36, accel_window=18, trend_ma=240,
                 min_gap=192, stop_pct=2.0, take_profit_pct=8.0),
    'SOL':  dict(velocity_window=36, accel_window=18, trend_ma=200,
                 min_gap=144, stop_pct=2.0, take_profit_pct=8.0),
    'NEAR': dict(velocity_window=36, accel_window=18, trend_ma=200,
                 min_gap=144, stop_pct=2.0, take_profit_pct=8.0),
    'ARB':  dict(velocity_window=36, accel_window=18, trend_ma=200,
                 min_gap=144, stop_pct=2.0, take_profit_pct=8.0),
}
INIT_CASH = 250.0
LEVERAGE = 5
...
for coin in COINS:
    ...
    seg_labels = [
        f"{s.index[0].strftime('%m/%d')}-{s.index[-1].strftime('%m/%d')}" for s in segs
    ]

    for seg_i, (seg, label) in enumerate(zip(segs, seg_labels)):
...
for coin in COINS:
    p = PARAMS[coin]
    print(f"\n{'─' * W}")
    print(f"  {coin}-USDT  |  vel_w={p['velocity_window']}, acc_w={p['accel_window']}, "
          f"trend_ma={p['trend_ma']}, gap={p['min_gap']}, "
          f"stop={p['stop_pct']}%, tp={p['take_profit_pct']}%")
    print(f"{'─' * W}")
    hdr = (f"  {'Segment':^24} {'#Tr':>4} {'WR%':>6} {'Ret%':>8} "
           f"{'Sharpe':>7} {'MaxDD%':>7} {'5xRet%':>9} {'5xP&L$':>9}")
    print(hdr)
    print(f"  {'─'*24} {'─'*4} {'─'*6} {'─'*8} {'─'*7} {'─'*7} {'─'*9} {'─'*9}")

    coin_df = df[df['coin'] == coin]
    for _, row in coin_df.iterrows():
        print(f"  {row['segment']:^24} {row['trades']:>4} "
    ...
    tot = coin_df.agg({
        'trades': 'sum', 'win_rate': 'mean', 'return_pct': 'sum',
        'sharpe': 'mean', 'lev_return_pct': 'sum', 'lev_profit_usd': 'sum',
    })
    print(f"  {'TOTAL':^24} {int(tot['trades']):>4} "
...
for coin in COINS:
    ...
    grand_pnl += tot_pnl
    print(f"  {coin:^8} {tot_tr:>6} {avg_wr:>7.1f} {tot_ret:>+9.2f} "
          f"{avg_sh:>9.2f} {tot_lev:>+10.2f} {tot_pnl:>+10.2f}")
