...
async def demo_okx_native() -> None:
    ...
    for f in funding[:3]:
        logger.info(
            f"  时间: {f['fundingTime']} | 费率: {f['fundingRate']} | "
            f"实际费率: {f.get('realizedRate', 'N/A')}"
        )
    ...
    if oi:
        logger.info(
            f"  合约: {oi[0]['instId']} | 持仓量: {oi[0]['oi']} | "
            f"持仓额: {oi[0].get('oiCcy', 'N/A')}"
        )
    ...
    if current:
        logger.info(
            f"  当前费率: {current.get('fundingRate', 'N/A')} | "
            f"下次费率: {current.get('nextFundingRate', 'N/A')}"
        )
