...
# ── 本地模拟盘前向测试(P2;Firstrade 无 paper,改本地按真价模拟)──
paper-forward:  # 每交易日一轮:warm 价→入场→退出→导出(盘后 17:30 ET 调度;退出码 0/2=ok,3=停)
	uv run python scripts/paper_forward.py $(if $(DATE),--date $(DATE),)

paper-backfill:  # ①一次性回填种子跑:近 7 天真实候选按真实历史价跑出首份 paper 结果
	uv run python scripts/paper_backfill.py
