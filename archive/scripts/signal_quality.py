...
for lo, hi in [(0, 35), (35, 45), (45, 55), (55, 65), (65, 80)]:
    ...
    if len(sub) >= 5:
        print(
            f"  RSI {lo:2d}-{hi:2d}: {len(sub):3d} trades | win:{sub['win'].mean() * 100:4.0f}% | avg ret:{sub['ret'].mean():+.2f}%"
        )
...
for lo, hi in [(0, 1), (1, 2), (2, 4), (4, 8), (8, 30)]:
    ...
    if len(sub) >= 5:
        print(
            f"  str {lo:2d}-{hi:2d}%: {len(sub):3d} trades | win:{sub['win'].mean() * 100:4.0f}% | avg ret:{sub['ret'].mean():+.2f}%"
        )
